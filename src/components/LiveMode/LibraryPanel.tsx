...
export function LibraryPanel({ side, decklist, library, ready, onImport }: LibraryPanelProps) {
    const [dialogOpen, setDialogOpen] = useState(false);
    const [query, setQuery] = useState('');
    const { setNodeRef, isOver } = useDroppable({ id: `lib-${side}` });

    const q = query.toLowerCase();
    const visibleCards = q ? library.filter(({ card }) => card.name.toLowerCase().includes(q)) : library;
